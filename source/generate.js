...
// Magic Sorting System v2
// (c) 2021 Demian Welker, MIT License
...
config.groups.forEach( function(group) {
	...
	if (items && items.length && target) {
		...
		fs.writeFileSync( group_func_file, 
			'execute as @s if entity @e[type=minecraft:item_frame,nbt={ItemRotation:3b,Item:{id:"' + target + '"}},distance=0..' + config.max_teleport_distance + '] run teleport @s @e[limit=1,sort=random,type=minecraft:item_frame,nbt={ItemRotation:3b,Item:{id:"' + target + '"}},distance=0..' + config.max_teleport_distance + ']' + "\n" +
			'execute as @s unless entity @e[type=minecraft:item_frame,nbt={ItemRotation:3b,Item:{id:"' + target + '"}},distance=0..' + config.max_teleport_distance + '] run ' + fallback_action + "\n"
		);
...
//Add one command which sorts all item ids which are not in the config file to the misc group and its final fallback
//Prevent items which are new or for some other reasone not in mss from despawning
var dw_itemId_checks = [];
config.groups.forEach( function(group) {
	if (group.items && group.items.length) {
		group.items.forEach( function(item_id) {
			dw_itemId_checks.push('nbt=!{Item:{id:"' + item_id + '"}}');	
		});
	}
});
sort_lines.push("");
sort_lines.push('execute as @s if entity @s[type=item,' + dw_itemId_checks.join(",") + '] run function mss:sort_misc');
